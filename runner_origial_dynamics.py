...
if __name__ == '__main__':
    ...
    for run in range(arglist.num_runs): #arglist.num_runs
        ...
        while num_of_game_plays <= arglist.max_num_game_plays:
            ...
            piirl = TEST(data_expert=trajectories, env=env, horizon=arglist.horizon, device=arglist.device,num_tra = arglist.num_traj, num_game = num_of_game_plays)
            ...
            piirl.update_mean_field_interval = int(0.4 * arglist.max_epoch)
            npiirl.update_mean_field_interval = int(0.4 * arglist.max_epoch)
            ...
            # 例如，重新创建一个Expert对象(可使用不同随机种子)：
            # validation_expert = Expert(env=env, horizon=arglist.horizon)
...
#
#     # performance comparison with varying number of samples
#     results = pd.DataFrame(columns=['samples', 'method', 'Difference_return', 'Dev_MF', 'Dev_Policy'])
#     for run in range(arglist.num_runs): #arglist.num_runs
#         print('===============Run: #' + str(run) + '================')
#         num_of_game_plays = 1
#         while num_of_game_plays <= arglist.max_num_game_plays:
#             print('Game Plays: ' + str(num_of_game_plays))
#             trajectories = expert.generate_trajectories_from_policy_flow(num_of_game_plays, arglist.num_traj,
#                                                                          expert.p_flow, expert.mf_flow)
#
#             piirl = TEST(data_expert=trajectories, env=env, horizon=arglist.horizon, device=arglist.device, num_tra=arglist.num_traj, num_game=num_of_game_plays)
...
#                   float(npiairl_dev_mf), float(npiairl_dev_p)],
#                  [num_of_game_plays, 'EXPERT', 0.0, 0.0, 0.0]],
#                 columns=['samples', 'method', 'Difference_return', 'Dev_MF', 'Dev_Policy'])
#
#             results = pd.concat([results, new_data], ignore_index=True)
#
#             piirl.save_model(model_save_path + 'pimfirl_' + str(num_of_game_plays) + '_' + str(run) + '.pt')
