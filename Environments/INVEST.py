...
class Env(Environment):
        ...
        for quality_level in range(self.state_count):
            ...
            if mean_quality <= self.quality_threshold:
                ...
                if quality_level == self.max_quality_level:
                    ...
                    level_bound = quality_level + math.ceil((self.state_count - quality_level) / 2) - 1
                    ...
                    if level_bound > quality_level:
                        if (self.state_count - quality_level) % 2 == 0:
    ...
    def dynamics(self, state, action, mean_field):
        if int(action.val[0]) == 1:
            mean_quality = self.mean_quality(mean_field)
            if mean_quality < self.quality_threshold:
                s = int(np.random.choice(np.array(range(state.val[0], self.state_count))))
